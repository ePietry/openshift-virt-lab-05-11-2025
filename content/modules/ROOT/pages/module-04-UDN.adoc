...
Alors que la méthode NAD (NetworkAttachmentDefinition) de la section précédente nous a permis de nous connecter à un pont (`br-flat`), elle laissait l'**IPAM** (IP Address Management) vide (`"ipam": {}`). La VM dépendait d'un DHCP externe ou d'une configuration statique manuelle.
