...
La création d'un UDN ne se fait pas via une `NetworkAttachmentDefinition` (NAD), mais en combinant deux concepts via la creation d'un objet `UserDefinedNetwork` qui définit le réseau L2 lui-même.

=== Définir l'objet UserDefinedNetwork
Pour cet exercice, **un namespace lié à un UDN a été créé pour chaque participant.**
Observons néanmoins les caractéristique de votre UDN.

. Dans le menu de navigation de gauche, cliquez sur *Networking* -> *UserDefinedNetworks*.
+
image::2025_spring/module-09-networking/40.png[link=self, window=blank, width=70%]
+
. Dans la liste déroulante *Project* en haut, sélectionnez le projet *udn-project-X*.
+
image::2025_spring/module-09-networking/41.png[link=self, window=blank, width=70%]
+
. Cliquez sur l'UDN correspondant à votre user afin d'en examiner les caractéristiques.
+
image::2025_spring/module-09-networking/42.png[link=self, window=blank, width=70%]
+
. Examinon maintenat notre UDN
+
image::2025_spring/module-09-networking/43.png[link=self, window=blank, width=80%]
+

Nous pouvons notamment observer :
. Le nom de cet objet UDN.
. Le Namespace où l'UDN va être l'utilisé
. La topologie Définit : L2 à plat (un simple switch).
. L'indication que ce réseau est conçu pour être un réseau "primaire" (par opposition à un réseau secondaire).
. L'utilisation de l'IMPAM, qui garantit que les adresses IP des VMs/Pods seront persistantes.
---
