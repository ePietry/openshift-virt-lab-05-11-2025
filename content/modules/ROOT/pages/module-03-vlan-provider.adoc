...
Pour valider notre configuration, nous allons créer deux nouvelles VMs, `fedora-bridged-1` et `fedora-bridged-2`. Contrairement aux VMs précédentes, nous allons les connecter *uniquement* au réseau d'entreprise pour simuler des machines qui ne doivent pas être exposées au réseau d'entreprise.
...
. Assurez-vous d'être dans le même projet où vous avez créé la NAD (projet-X).
. Cliquez sur *Create* -> *From Template*.
. Cliquez sur la tuille *Fedora VM*.
. Dans l'assistant, remplissez la section *General* :
    * *Name:* `fedora-bridged-1`
. Cliquez sur *Customise Machine*.
+
...
+
. L'interface par défaut *Pod Networking* est présente. Cliquez sur le menu à trois points à droite de cette interface et sélectionnez *Delete* pour la supprimer.
...
. Vérifiez que *seule* l'interface `corp-network` est listée pour votre VM.
+
...
. Vous verrez une seul interface listée :
    * L'interface `corp-network` qui affiche une ip dans le subnet 192.168.127.0/24. Cette IP à été attribué par le DHCP de notre réseau d'entreprise.
...
Maintenant, nous allons effectuez un clique droit sur la machine fedora-bridged-1 et cliquer sur clone. Nous lui donnerons le nom de fedora-bridged-2

Maintenant que nos deux VMs sont créées et ne sont connectées qu'au réseau de l'entreprise, vérifions qu'elles peuvent communiquer entre elles.
...
[source,sh]
----
$ ping -c 3 192.168.127.51   # <-- Remplacez par l'IP réelle de fedora2
...
. Dans la liste principale *VirtualMachines*, regardez l'onglet *Overview*. Vous devriez voir les adresses IP attribuées à chaque VM.
== Résumé
