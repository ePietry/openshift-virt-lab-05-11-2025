...
Pour valider notre configuration, nous allons créer deux nouvelles VMs, `fedora1` et `fedora2`. Contrairement aux VMs précédentes, nous allons les connecter *uniquement* au réseau d'entreprise pour simuler des machines qui ne doivent pas être exposées au réseau d'entreprise.
