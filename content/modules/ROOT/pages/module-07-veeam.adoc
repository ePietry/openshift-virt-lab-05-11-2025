...
. Vous devriez voir une route nommée *k10-route*. Cliquez sur l'URL dans la colonne *Location*.
+
image::2025_spring/kasten/3.png[link=self, window=blank, width=100%]

. Une nouvelle fenêtre s'ouvrira avec le tableau de bord Kasten K10. Vous pouvez désormais vous identifier avec vos logins OpenShift.
+
image::2025_spring/kasten/4.png[link=self, window=blank, width=50%]

. Si nécessaire, autorisez l’accès à Kasten à votre nom d'utilisateur
+
image::2025_spring/kasten/9.png[link=self, window=blank, width=100%]

== Création et Exécution d'une Politique de Sauvegarde
Nous avons maintenant accès à la console Veeam Kasten.

Nous allons maintenant créer une Policy qui dit à Kasten quoi sauvegarder et comment.

. Commençons par cliquer sur la tuile *application* dans le dashboard 
+
image::2025_spring/kasten/5.png[link=self, window=blank, width=100%]


. Dans le tableau de bord sélectionnez le menu à trois points de notre *projet-X* et cliquez sur *Create a policy*
+
image::2025_spring/kasten/6.png[link=self, window=blank, width=100%]

. Remplissez le formulaire pour notre politique de sauvegarde :
    * *Name:* Nous pouvons le laisser par default
    * *Backup Frequency:* *Daily*
    * Sélectionnez *Enable Backups via Snapshot Exports* et sélectionnez le stockage *s3-odf*
    * Laissez les autres options par défaut et cliquez sur *Submit*
+
image::2025_spring/kasten/10.png[link=self, window=blank, width=70%]

. Cliquez sur *Create Policy*.

. La politique est maintenant créée, mais elle n'a pas encore été exécutée. Sur la ligne de votre nouvelle politique (`projet-x-backup`), cliquez sur l'icône à droite puis sur *Run Once*.
+
image::2025_spring/kasten/11.png[link=self, window=blank, width=100%]

. Confirmez en cliquant sur *Yes, continue* dans la fenêtre contextuelle.
...
+
image::2025_spring/kasten/12.png[link=self, window=blank, width=100%]
...
. Cliquez sur le menu *Actions* (dans le coin supérieur droit) et sélectionnez *Stop*, puis *Delete*.
+
image::2025_spring/kasten/13.png[link=self, window=blank, width=100%]
...
Utilisons maintenant Kasten pour restaurer notre VM à partir du point de sauvegarde.

. Retournez au tableau de bord Kasten K10.

. Cliquez sur *Restore Points* dans le menu. Vous devriez toujours voir votre *projet* dans la liste.

. Cliquez sur le nom de votre *projet*.

. Vous verrez une liste de *Restore Points*. Localisez le point de restauration valide que nous avons créé à l'étape précédente.
+
image::2025_spring/kasten/15.png[link=self, window=blank, width=100%]
...
+
image::2025_spring/kasten/16.png[link=self, window=blank, width=100%]
...
+
image::2025_spring/kasten/14.png[link=self, window=blank, width=100%]
