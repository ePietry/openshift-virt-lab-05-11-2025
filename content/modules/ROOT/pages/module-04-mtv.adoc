...
The application will display https://console-openshift-console.apps.rosa.rosa-plt6q.zs4d.p3.openshiftapps.com/k8s/all-namespaces/rbac.authorization.k8s.io~v1~Rolea visitor counter, and the hostname of the webserver that returned your request.
...
* *User:* `ocpvirt-dlqxp@infra`
* *Password:* `t256.DWHSmwoM8`
+
...
[Sidebar]
****
If you do not see the Providers, select the *mtv-userX* Project by:
...
+
link:{ocp_console_url}/k8s/ns/openshift-mtv/forklift.konveyor.io%7Ev1beta1~Plan[Link to Create Migration Plan^]
+
...
* Storage map: *{ocp_mtv_storage_map}*
ifeval::["{ocp_mtv_storage_map}" == "ocs-storagecluster-ceph-rbd-virtualization"]
+
...
Make sure to change it to *{ocp_mtv_storage_map}*.
endif::[]
